#!/usr/bin/env python3
"""
删除processed_files表中2025-10-03 15:00之后的所有数据

功能：
1. 连接neon数据库
2. 查询processed_files表中2025-10-03 15:00之后的数据
3. 删除这些数据
4. 显示清理统计信息

使用方法：
python3 program/cleanup_processed_files_after_3pm.py
python3 program/cleanup_processed_files_after_3pm.py --dry-run  # 只查看不删除
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('cleanup_processed_files.log'),
...
class ProcessedFilesCleaner:
    ...
    def get_data_stats(self):
        ...
        try:
            # 获取总数据量
            self.cursor.execute("SELECT COUNT(*) as total_count FROM processed_files")
            ...
            WHERE (processed_time AT TIME ZONE 'UTC' AT TIME ZONE 'US/Pacific')::date = '2025-10-03'
            AND (processed_time AT TIME ZONE 'UTC' AT TIME ZONE 'US/Pacific')::time >= '15:00:00'
            """
            ...
            sample_query = """
            SELECT id, folder_name, csv_filename, file_type, processed_time,
                   (processed_time AT TIME ZONE 'UTC' AT TIME ZONE 'US/Pacific') as pst_time
            FROM processed_files 
            WHERE (processed_time AT TIME ZONE 'UTC' AT TIME ZONE 'US/Pacific')::date = '2025-10-03'
            AND (processed_time AT TIME ZONE 'UTC' AT TIME ZONE 'US/Pacific')::time >= '15:00:00'
    ...
    def cleanup_after_3pm_data(self, dry_run=False):
        ...
        try:
            ...
            for row in stats['sample_data']:
                logger.info(f"  ID:{row['id']} {row['folder_name']}/{row['csv_filename']} ({row['file_type']}) - {row['pst_time']}")
            ...
            # 获取删除后的统计信息
            self.cursor.execute("SELECT COUNT(*) as remaining_count FROM processed_files")
    ...
    def run(self, dry_run=False):
        """运行清理程序"""
        logger.info("🚀 开始删除processed_files表中2025-10-03 15:00之后的数据")
...
def main():
    parser = argparse.ArgumentParser(description='删除processed_files表中2025-10-03 15:00之后的所有数据')
