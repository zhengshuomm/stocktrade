...
def main():
    ...
    try:
        ...
        # 跨日处理：如果最新两份文件的日期不同，需要查找前一天最后一个文件用于股票趋势判断
        is_cross_day = latest_ts.strftime('%Y%m%d') != previous_ts.strftime('%Y%m%d')
        if is_cross_day:
            print("检测到跨日数据，将查找前一天最后一个文件用于股票趋势判断")
            # 查找前一天最后一个文件用于股票趋势判断
        ...
        if args.discord:
            ...
            try:
                ...
                # 使用新的模块化Discord发送器
                import asyncio
                asyncio.run(send_oi_outliers(out_df, args.folder, time_range, stock_prices, None, signal_type_stats, out_path))
